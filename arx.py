...
class Arx:
    ...
    def __init__(self, minimum_input_delay: int, inputs_maximum_delays: list, output_maximum_delay: int, learning_inputs: list, learning_output: list, offset=False):
        ...
        self.parameters = (numpy.linalg.inv(input_measured.transpose() * input_measured) * input_measured.transpose() * output_measured).squeeze().tolist()[0]
        parameters_index = 0
    ...
    def simulate(self, inputs=None, output=None, cycle=None):
        ...
        print( 'RMSE= %f' % ((sum([(abs(error))**2 for error in output_errors]) / len(output_errors)) **0.5))
...
def plot_result(output_estimated: list, output_measured: list, datetime: list, inputs_measured: 'list  of lists'=None):
    ...
    axes[0].set_xlim(left=0, right=len(datetime))
